...
class NeuralNetwork():
    def __init__(self, k):
        ...
        print "Weights Shape", self.synaptic_weigths.shape
    ...
    def train(self, trainset_inputs, trainset_labels, number_of_iterations):
        for iteration in xrange(number_of_iterations):
            # pass the training set through the neural net
            predicted_label = self.predict(trainset_inputs)
            # calculate the error
            error = trainset_labels - predicted_label
            # multiply the error by the input and again by the gradient of the sigmoid curve
            adjusment = dot(trainset_inputs.T, error * self.__sigmoid_derivative(predicted_label))
    ...
    def score(self, testset_inputs, testset_labels):
        predictions = self.predict(testset_inputs)
        return sum(testset_labels - predictions)
